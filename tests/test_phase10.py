...
* ``scripts/build_sidecar.py`` writes development entry-point stubs under
  ``apps/desktop/sidecar/`` without claiming they are bundle inputs.
"""
...
def test_tauri_bundle_stays_inactive_until_real_sidecars_exist() -> None:
    config_path = REPO_ROOT / "apps" / "desktop" / "src-tauri" / "tauri.conf.json"
    config = json.loads(config_path.read_text(encoding="utf-8"))
    assert config["bundle"]["active"] is False
    assert not config["bundle"].get("externalBin")
